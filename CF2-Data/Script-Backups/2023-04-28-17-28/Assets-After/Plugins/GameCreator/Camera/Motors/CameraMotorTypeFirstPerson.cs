namespace GameCreator.Camera
{
	...
	public class CameraMotorTypeFirstPerson : ICameraMotorType 
	{
        ...
        private float rotationZVelocity = 0.0f;
        ...
        public override void EnableMotor()
        {
            ...
            this.cursorLock = ControlFreak2.CFCursor.lockState;
        }
		...
		public override Vector3 GetDirection (CameraController camera, bool withoutSmoothing = false)
		{
            ...
            this.rotationZ = Mathf.SmoothDampAngle(
                this.rotationZ, this.targetRotationZ,
                ref this.rotationZVelocity, this.smoothRotation
            );
            ...
            if (float.IsNaN(this.rotationZ)) this.rotationZ = this.targetRotationZ;
            ...
            Quaternion quaternionZ = Quaternion.AngleAxis(this.rotationY, Vector3.forward);

			return (quaternionX * quaternionY * quaternionZ) * Vector3.forward;
		}
        ...
        private void RotationInput()
        {
            if (Application.isMobilePlatform)
            {
                ...
                {
                    ControlFreak2.InputRig.Touch touch = ControlFreak2.CF2Input.touches[i];
            ...
            else
            {
                ...
                else if (this.rotateInput == RotateInput.HoldLeftMouse && ControlFreak2.CF2Input.GetMouseButton(0)) inputConditions = true;
                else if (this.rotateInput == RotateInput.HoldRightMouse && ControlFreak2.CF2Input.GetMouseButton(1)) inputConditions = true;
                else if (this.rotateInput == RotateInput.HoldMiddleMouse && ControlFreak2.CF2Input.GetMouseButton(2)) inputConditions = true;
                else inputConditions = false;

                if (inputConditions)
                {
                    float axisX = ControlFreak2.CF2Input.GetAxisRaw(INPUT_MOUSE_X);
                    float axisY = ControlFreak2.CF2Input.GetAxisRaw(INPUT_MOUSE_Y);

                    if (this.cursorLock != ControlFreak2.CFCursor.lockState && !Mathf.Approximately(axisX + axisY, 0f))
                    {
                        this.cursorLock = ControlFreak2.CFCursor.lockState;
                        return;
                    }
